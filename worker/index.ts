...
const songUpdateSchema = z.object({
  ...
  vocalCredit: z.string().trim().max(200).nullable(),
  lyricistCredit: z.string().trim().max(200).nullable(),
  composerCredit: z.string().trim().max(200).nullable(),
  arrangerCredit: z.string().trim().max(200).nullable(),
...
app.put("/api/songs/:id", async (c) => {
  ...
  const updated = await c.env.DB.prepare(
    `UPDATE songs SET title = ?, bpm = ?, time_signature_numerator = ?,
      ...
      vocal_credit = ?, lyricist_credit = ?, composer_credit = ?, arranger_credit = ?,
    ...
    data.initialKey,
    data.sourceUrl || null,
    data.vocalCredit?.trim() || null,
    data.lyricistCredit?.trim() || null,
    data.composerCredit?.trim() || null,
    data.arrangerCredit?.trim() || null,
    data.status,
    publishedAt,
    c.req.param("id"),
    data.version,
...
async function loadSong(db: D1Database, slug: string, user: SessionUser | null): Promise<Song | null> {
  ...
  return {
    ...
    vocalCredit: row.vocal_credit ? String(row.vocal_credit) : null,
    lyricistCredit: row.lyricist_credit ? String(row.lyricist_credit) : null,
    composerCredit: row.composer_credit ? String(row.composer_credit) : null,
    arrangerCredit: row.arranger_credit ? String(row.arranger_credit) : null,
